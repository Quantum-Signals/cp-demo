...
curl -X POST -H "Authorization: Bearer ${JWT_TOKEN}" -H "Content-Type: application/json" -d '{"name":"Offline Topic Partitions","clusterId":"default","condition":"GREATER_THAN","longValue":"0","lagMs":"60000","brokerClusters":{"brokerClusters":["'$clusterId'"]},"brokerMetric":"OFFLINE_TOPIC_PARTITIONS"}' http://localhost:9021/2.0/alerts/triggers
